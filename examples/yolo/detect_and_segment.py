...
def vis_detections(image,result,result2) :
    w = image.shape[1]
    h = image.shape[0]
    seg_w = result2.shape[1]
    seg_h = result2.shape[2]
    pic1 = cv2.resize(result2[0,:,:], (w, h), interpolation=cv2.INTER_LINEAR)
    pic2 = cv2.resize(result2[1,:,:], (w, h), interpolation=cv2.INTER_LINEAR)
    for i in range(h):
      for j in range(w):
        if(pic1[i,j]>0.45) :
          image[i,j,0]=255
